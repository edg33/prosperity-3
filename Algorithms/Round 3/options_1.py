    ...
    def run(self, state: TradingState):
        ...
        for product, order_depth in state.order_depths.items():
            ...
            # ================================
                ...
                max_rock_position = 400
                max_voucher_position = 200
                fair_value_margin = 50  # buffer around intrinsic value
                volcanic_products = [
                ...
                rock_orders = state.order_depths.get("VOLCANIC_ROCK")
                if rock_orders:
                    best_ask = min(rock_orders.sell_orders.keys(), default=None)
                    best_bid = max(rock_orders.buy_orders.keys(), default=None)
                    if best_ask and best_bid:
                        rock_price = (best_ask + best_bid) / 2
                    elif best_ask:
                        rock_price = best_ask
                    else:
                        rock_price = best_bid
                else:
                    rock_price = None  # cannot proceed without a reference rock price
                ...
                for product, order_depth in state.order_depths.items():
                    ...
                    if product == "VOLCANIC_ROCK":
                        continue  # skip, used only for reference pricing

                    for name, strike_price in volcanic_products:
                        if product == name and rock_price is not None:
                            intrinsic_value = max(0, rock_price - strike_price)
                            fair_buy_price = intrinsic_value - fair_value_margin
                            fair_sell_price = intrinsic_value + fair_value_margin
                            available_buy = max_voucher_position - current_position
                            available_sell = max_voucher_position + current_position

                            print(f"[Time {state.timestamp}] Product: {product}; Rock Price: {rock_price:.2f}; "
                                f"Strike: {strike_price}; Intrinsic: {intrinsic_value}; "
                                f"Best Bid: {best_bid}; Best Ask: {best_ask}; "
                                f"Fair Buy ≤ {fair_buy_price}, Fair Sell ≥ {fair_sell_price}; Position: {current_position}")

                            # Buy logic
                            if best_ask is not None and best_ask < fair_buy_price and available_buy > 0:
                                order_size = min(available_buy, -order_depth.sell_orders[best_ask])
                                if order_size > 0:
                                    orders.append(Order(product, best_ask, order_size))
                                    print(f"--> {product}: Placing BUY order for {order_size} units at {best_ask}", end="; ")

                            # Sell logic
                            if best_bid is not None and best_bid > fair_sell_price and available_sell > 0:
                                order_size = min(available_sell, order_depth.buy_orders[best_bid])
                                if order_size > 0:
                                    orders.append(Order(product, best_bid, -order_size))
                                    print(f"--> {product}: Placing SELL order for {order_size} units at {best_bid}", end="; ")
