...
class Trader:
    def run(self, state: TradingState):
        ...
        for product, order_depth in state.order_depths.items():
            ...
            # ================================
            # SQUID_INK Trading Logic (Multiple MAs)
            # ================================
            elif product == "SQUID_INK":
                # For SQUID_INK we use two moving averages: short-term and long-term, similar to KELP
                squid_data = trader_data.get(product, {"short_ma": mid_price, "long_ma": mid_price})
                short_ma = squid_data.get("short_ma", mid_price)
                long_ma = squid_data.get("long_ma", mid_price)
                ...
                if updated_short_ma > updated_long_ma:
                    ...
                    if best_bid is not None and best_bid > updated_short_ma and available_sell > 0:
                        ...
                        if order_size > 0:
                            orders.append(Order(product, best_bid, -order_size))
                            print(f"--> SQUID_INK: Bearish signal - Placing SELL order for {order_size} units at {best_bid}", end=";")
                
                # Update SQUID_INK data in trader_data
                trader_data[product] = {"short_ma": updated_short_ma, "long_ma": updated_long_ma}

            # If product is neither RAINFOREST_RESIN, KELP, nor SQUID_INK, leave orders empty.
