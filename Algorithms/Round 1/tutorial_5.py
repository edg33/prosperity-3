...
class Trader:
    def run(self, state: TradingState):
        ...
        for product, order_depth in state.order_depths.items():
            ...
            # ================================
            # SQUID INK Trading Logic (Multiple MAs)
            # ================================
            elif product == "SQUID_INK":
                # Retrieve previous data for SQUID_INK (or initialize lists)
                squid_ink_data = trader_data.get(product, {"short_prices": [], "long_prices": [], "short_long": False})
                short_prices = squid_ink_data.get("short_prices", [])
                long_prices = squid_ink_data.get("long_prices", [])
                short_lower_before = squid_ink_data.get("short_long", False)
                ...
                # Compute the short and long moving averages (MAs)
                short_ma = sum(short_prices) / len(short_prices) if short_prices else mid_price
                long_ma = sum(long_prices) / len(long_prices) if long_prices else mid_price
                ...
                sell_signal = (short_ma < long_ma) and not short_lower_before
                if sell_signal:
                    squid_ink_data["short_long"] = True

                # Buy signal (Can mess around with adding a percentage here)
                buy_signal = (short_ma > long_ma) and short_lower_before
                if buy_signal:
                    squid_ink_data["short_long"] = False
