    ...
    def run(self, state: TradingState):
        ...
        for product, order_depth in state.order_depths.items():
            ...
            if product == "RAINFOREST_RESIN":
                ...
                # Enhanced BUY logic: Consider both mean reversion and momentum
                if best_ask is not None and best_ask < historical_mean * 0.995 and available_buy > 0:
                    # More aggressive buying when price is significantly below mean
                ...
                # Enhanced SELL logic: Consider both mean reversion and momentum
                if best_bid is not None and best_bid > historical_mean * 1.005 and available_sell > 0:
                    # More aggressive selling when price is significantly above mean
            ...
            # ================================
                ...
                # Enhanced signal generation using moving average crossovers:
                # Bullish signal if short MA is above long MA, bearish if below.
                if updated_short_ma > updated_long_ma * 1.002:  # Added 0.2% threshold for stronger signals
                    # Bullish: if best ask is below the short MA, consider buying.
                    if best_ask is not None and best_ask < updated_short_ma * 0.998 and available_buy > 0:
                        order_size = min(available_buy, -order_depth.sell_orders[best_ask])
                        if order_size > 0:
                            orders.append(Order(product, best_ask, order_size))
                            print(f"--> KELP: Bullish signal - Placing BUY order for {order_size} units at {best_ask}", end=";")
                elif updated_short_ma < updated_long_ma * 0.998:  # Added 0.2% threshold for stronger signals
                    # Bearish: if best bid is above the short MA, consider selling.
                    if best_bid is not None and best_bid > updated_short_ma * 1.002 and available_sell > 0:
                        order_size = min(available_sell, order_depth.buy_orders[best_bid])
                        if order_size > 0:
                            orders.append(Order(product, best_bid, -order_size))
